  ...
  public function __invoke(Request $request): Responsable
  {
    ...
    $status = Broker::find()->reset(
      ...
      function ($user) use ($request) {
        app(ResetsUserPasswordsInterface::class)->reset($user, $request->all());
    ...
    return $status == Password::PASSWORD_RESET
      ? app(PasswordResetInterface::class, ['status' => $status])
      : app(FailedPasswordResetInterface::class, ['status' => $status]);
  }
}
