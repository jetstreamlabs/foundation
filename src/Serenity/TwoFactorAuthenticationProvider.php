...
class TwoFactorAuthenticationProvider implements TwoFactorAuthenticationProviderInterface
{
  /**
  ...
  {
    if (is_int($customWindow = config('serenity-options.two-factor-authentication.window'))) {
      $this->engine->setWindow($customWindow);
    }

    $timestamp = $this->engine->verifyKeyNewer(
      $secret, $code, optional($this->cache)->get($key = 'serenity.2fa_codes.'.md5($code))
    );
