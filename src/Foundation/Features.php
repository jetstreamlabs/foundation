...
class Features
{
  ...
  {
    return in_array($feature, config('serenity.features', []));
  }
  ...
  public static function optionEnabled(string $feature, string $option)
  {
    return static::enabled($feature) &&
      config("serenity-options.{$feature}.{$option}") === true;
  }
  ...
  {
    if (! empty($options)) {
      config(['serenity-options.teams' => $options]);
    }
  ...
  {
    if (! empty($options)) {
      config(['serenity-options.two-factor-authentication' => $options]);
    }
